...
class Zotero
{
	...
	public:
		...
		struct Request
		{
			static constexpr const char GetAllItems[] = "/collections/DJI5I838/items?format=json&include=data,bib,citation&style=kritische-ausgabe&limit=100"; ///< The zotero request to get all items in the zotero library from zotero
			static constexpr const char GetAllPillars[] = "/collections/DJI5I838/collections?format=json"; ///< The zotero request to get all collections from the zotero api
			...
			static std::string GetAllItemsFromCollection(std::string collKey); ///<Get all the top level items in a specific zotero collection
		};
